...
import { type InvalidNameError, type InvalidEmailError, type InvalidPasswordError } from '@/users/entities/errors'
...
describe('User domain entity', () => {
  ...
  test('should not create user with invalid email address', () => {
    ...
    const error = User.create({ name: 'Any Name', email: invalidEmail, password: 'Xpassword1*' }) as InvalidEmailError
  ...
  test('should not create user with invalid name (too few characters)', () => {
    const invalidName = 'O'
    const error = User.create({ name: invalidName, email: 'any@email.com', password: 'Xpassword1*' }) as InvalidNameError
  ...
  test('should not create user with invalid password (no special character)', () => {
    ...
    const error = User.create({ name: 'Any Name', email: 'any@email.com', password: invalidPassword }) as InvalidPasswordError
